...
export class WarehouseService extends TypeOrmCrudService<Warehouse> {
  ...
  async getOne(req: CrudRequest): Promise<Warehouse> {
    ...
    if (warehouse) {
      warehouse.products =
        await this.warehouseProductService.getCountForWarehouse(warehouse.id);
      warehouse.productsAvailable =
        await this.warehouseProductService.getCountForWarehouse(
          warehouse.id,
          true,
        );
    }
