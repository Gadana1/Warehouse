      ...
      if (!(await this.userService.count())) {
        ...
        if (user) {
          ...
          if (!(await this.roleService.count())) {
            // User Role
            this.roleService.create({ name: "User", permissions: [
              Permission.AdminProduct,
              Permission.AdminWarehouse,
              Permission.AdminWarehouseProduct,
            ]});
            // Admin Role
            role = await this.roleService.create({ name: "Administrator", permissions: [Permission.Admin] });
                    }
          else {
            role = await this.roleService.findOne({ where: { name: "Administrator" } });
          }
