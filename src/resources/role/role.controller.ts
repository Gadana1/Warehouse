import { Body, Controller, Delete, Get, HttpException, Put, UseInterceptors } from '@nestjs/common';
...
@Crud({
  ...
  routes: {
    createManyBase: {
      decorators: [
        Permissions(Permission.RoleCreateMany)
      ]
    },
    createOneBase: {
      decorators: [
        Permissions(Permission.RoleCreateOne)
      ]
    },
    updateOneBase: {
      decorators: [
        Permissions(Permission.RoleUpdateOne)
      ]
    },
    replaceOneBase: {
      decorators: [
        Permissions(Permission.RoleReplaceOne)
      ]
    },
    deleteOneBase: {
      decorators: [
        Permissions(Permission.RoleDeleteOne)
      ]
    },
    recoverOneBase: {
      decorators: [
    ...
    getManyBase: {
      decorators: [
        Permissions(Permission.RoleReadAll)
      ]
    },
    getOneBase: {
      decorators: [
        Permissions(Permission.RoleReadOne)
      ]
    }
  },
  // Modify query result
...
export class RoleController implements CrudController<Role>  {
  ...
  async addRole(@ParsedRequest() req: CrudRequest, @Body() dto: UpdateRolePermissionDto): Promise<void | Role> {
    if (dto) {
  ...
  @UseInterceptors(CrudRequestInterceptor)
  async removeRole(@ParsedRequest() req: CrudRequest, @Body() dto: UpdateRolePermissionDto): Promise<void | Role> {
    if (dto) {
